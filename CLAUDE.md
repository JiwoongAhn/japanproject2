...
- [x] 안정성 D-2(화면 전환 깜빡임=네비 배경 통일)·D-1(게시판/과제 FlatList 가상화)·B-1(헤더 뒤로가기/닫기 버튼 19곳 hitSlop) 적용, B-3 뒤로가기 제스처는 확정 버그 없어 보류 (2026-06-08)
...
- [x] **메일 내용 요약 표시** — 미리보기 모달에 과목/요약/마감/첨부 한눈에 (2026-06-03)
- [x] **과제 자동 추가 버튼** — 課題/小テスト 메일에서 prefill된 AssignmentAdd로 이동 (2026-06-03)
- [x] **메인 화면 요약 히어로 카드(今日のまとめ)** — 授業/課題/お知らせ 3칸 통합, 현재 시각 기준 지금·다음 수업 자동 표시 (2026-06-08)
- [x] **히어로 카드 후속 3건** — 授業 칸 상태 4분기(本日終了 등) / 公示→お知らせ 라벨 통일 / お知らせ 카운트를 manaba 미리보기 배지와 동일(콜백으로 끌어올림) (2026-06-08)
- [x] **시간표/과제 UI 수정 3건** — 일괄추가 복귀 다이얼로그 / 과제 제출 양방향 확인 / 시라바스 인앱 브라우저(WebBrowser) (2026-06-08)
- [x] **공지 既読 통합 + 스와이프 삭제** — 공지 탭=확인 시 자동 제거(push=DB 읽음, web=기기 숨김 목록), 홈 미리보기+전체 목록 왼쪽 스와이프 빨간 既読 버튼. App.js GestureHandlerRootView 래핑 (2026-06-08)
- [x] **일괄추가 색상 선택** — 미리보기에서 과목별 색 자동배정 + 색 동그라미 탭→팔레트 변경, color_index 저장 (2026-06-08)
- [x] **시간표 전체 삭제 버튼** — 時間割 화면 今日はNコマ 줄 우측 🗑全て削除, 확인 다이얼로그(元に戻せません) 후 user_id 기준 일괄 삭제 (2026-06-08)
- [x] **Microsoft 재연결 배지** — refresh_token 만료(invalid_grant) 감지 시 mail_subscriptions.needs_reauth=true → マイページ 경고 카드+빨간 再連携 버튼, 재연결 성공 시 리셋 (2026-06-09)
- [x] **refresh_token 암호화** — mail_subscriptions.ms_refresh_token 평문→AES-256-GCM(키=Supabase secret MAIL_TOKEN_ENC_KEY, DB 밖 보관). renew/oauth-exchange/webhook 3함수 적용, enc:v1: 접두사 레거시 평문 호환 (2026-06-09)
- [x] **학교 약관 컴플라이언스 2건** — ①면책고지: 학교 로그인 화면(Manaba/SchoolWebView) 하단 고정 안내바(非公式 앱 명시) UnofficialNotice 컴포넌트 ②User-Agent: 학교접속 WebView 4곳에 applicationNameForUserAgent로 `Unipas/1.0` 식별자 주입. 정리문서=`~/.claude/plans/school-tos-compliance.md` (2026-06-09)
